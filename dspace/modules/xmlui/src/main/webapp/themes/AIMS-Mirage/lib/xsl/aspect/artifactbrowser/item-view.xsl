...
<xsl:stylesheet
    ...
    <xsl:template name="itemSummaryView-DIM-fields">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$clause = 1">
                <!--<xsl:choose>-->
                <!--<xsl:when-->
                <!--test="descendant::text() and (count(dim:field[@element='title'][not(@qualifier)]) &gt; 1)">-->
                <!--&lt;!&ndash; display first title as h1 &ndash;&gt;-->
                <!--<h1>-->
                ...
                <!--</xsl:when>-->
                <!--<xsl:when-->
                <!--test="dim:field[@element='title'][descendant::text()] and count(dim:field[@element='title'][not(@qualifier)]) = 1">-->
                <!--<h1>-->
                <!--<xsl:value-of select="dim:field[@element='title'][not(@qualifier)][1]/node()"/>-->
                <!--</h1>-->
                <!--</xsl:when>-->
                ...
                <xsl:choose>
                    <xsl:when
                            test="dim:field[@element='identifier' and @qualifier='citation' and descendant::text()]">
                        <h1>
                            <xsl:value-of
                                    select="dim:field[@element='identifier' and @qualifier='citation'][1]/node()"/>
                        </h1>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:choose>
            ...
            <!-- Author(s) row -->
            <!--<xsl:when-->
            <!--test="$clause = 2 and (dim:field[@element='contributor'][@qualifier='author' and descendant::text()] or dim:field[@element='creator' and descendant::text()] or dim:field[@element='contributor' and descendant::text()])">-->
            ...
            <!-- identifier.uri row -->
            <xsl:when
                    test="$clause = 2 and (dim:field[@element='identifier' and @qualifier='uri' and descendant::text()])">
                <div class="simple-item-view-other">
            ...
            <!-- date.issued row -->
            <xsl:when
                    test="$clause = 3 and (dim:field[@element='date' and @qualifier='issued' and descendant::text()])">
                <div class="simple-item-view-other">
            ...
            <!--<xsl:when test="$clause = 5 and (dim:field[@element='identifier' and @qualifier='citation' and descendant::text()])">-->
            <!--<div class="simple-item-view-other">-->
            ...
            <!-- Description row -->
            <xsl:when test="$clause = 5 and (dim:field[@element='description' and not(@qualifier) and descendant::text()])">
                <div class="simple-item-view-description">
            ...
            <!-- relation uri row -->
            <xsl:when test="$clause = 6 and (dim:field[@element='relation' and @qualifier='uri' and descendant::text()])">
                <div class="simple-item-view-other">
    ...
    <xsl:template name="display-rights">
        ...
        <xsl:choose>
            <xsl:when
                    test="(not ($rights_context/@CONTEXTCLASS = 'GENERAL PUBLIC') and ($rights_context/rights:Permissions/@DISPLAY = 'true')) or not ($rights_context)">
    ...
    <xsl:template name='impact-altmetric'>
        <div id='impact-altmetric'>
            ...
            <script type="text/javascript"
                    src="{concat($scheme, 'd1bxh8uas1mnw7.cloudfront.net/assets/embed.js')}">&#xFEFF;
            </script>
            <div id='altmetric'
                ...
                <xsl:if test='$badge_popover'>
                    <xsl:attribute name='data-badge-popover'>
                        <xsl:value-of select='$badge_popover'/>
                    </xsl:attribute>
                </xsl:if>

                <xsl:variable name='badge_details' select='confman:getProperty("altmetric.details")'/>
                <xsl:if test='$badge_details'>
                    <xsl:attribute name='data-badge-details'>
                        <xsl:value-of select='$badge_details'/>
                    </xsl:attribute>
                </xsl:if>

                <xsl:variable name='no_score' select='confman:getProperty("altmetric.noScore")'/>
                <xsl:if test='$no_score'>
                    <xsl:attribute name='data-no-score'>
                        <xsl:value-of select='$no_score'/>
                    </xsl:attribute>
                </xsl:if>
                ...
                <xsl:choose>    <!-- data-doi data-handle data-arxiv-id data-pmid -->
                    ...
                    <xsl:when test='$identifier_handle'>
                        <xsl:attribute name='data-handle'>
                            <xsl:value-of select='$identifier_handle'/>
                        </xsl:attribute>
                    </xsl:when>
                </xsl:choose>
                &#xFEFF;
            ...
            <a>
                ...
                <xsl:if test="$plumx_data-popup">
                    <xsl:attribute name="data-popup">
                        <xsl:value-of select="$plumx_data-popup"/>
                    </xsl:attribute>
                </xsl:if>
